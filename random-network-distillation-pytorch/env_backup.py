    ...
    def run(self):
        ...
        while True:
            ...
            if self.is_render:
                #self.env.render()
                #print("rendering")
                yield self.env.render(mode='rgb_array')
            ...
            if done and 'Montezuma' in self.env_id:
            ...
            elif done and 'Breakout' in self.env_id:
                self.recent_rlist.append(self.rall)
...
class MarioEnvironment(Process):
    def __init__(
        ...
        self.history_size = history_size
        self.history = np.zeros([self.history_size, h, w])
        self.h = h
        self.w = w
    ...
    def run(self):
        ...
        while True:
            ...
            if done:
                ...
                self.history = self.reset()
                #self.writer.add_scalar('Episode/Current_X', info['x_pos'], self.episode)
...
class LegendOfZeldaEnvironment(Process):
    def __init__(
            ...
            life_done=False,
            h=84,
            w=84, sticky_action=True,
            p=0.25):
        super(LegendOfZeldaEnvironment, self).__init__()
        self.daemon = True
        self.env = retrowrapper.RetroWrapper(game="LegendOfZelda-Nes", 
    ...
    def run(self):
        ...
        while True:
            ...
            self.steps += 1
            if self.steps == 1000 and not done:
                done = True
                reward -= 5
                print("Did not finish room in 2500 steps")
            ...
            if done:
                self.recent_rlist.append(self.rall)
                print(
                    "[Episode {}({})] Step: {}  Reward: {}  Recent Reward: {}  Location: {}   Number of Keys:{}   Has Bow:{}".format(
                        self.episode,
                        self.env_idx,
                        self.steps,
                        self.rall,
